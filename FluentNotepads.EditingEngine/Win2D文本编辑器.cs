...
public class Win2DTextEditor : UserControl
{
    ...
    // ========== 渲染配置 ==========
    private CanvasTextFormat? _textFormat;  // 可空，在 CreateResources 中初始化
    ...
    public Win2DTextEditor()
    {
        try
        {
            System.Diagnostics.Debug.WriteLine("🚀 Win2DTextEditor 构造函数开始");
            
            // 检查环境（仅在第一次创建时）
            // Win2DDebugHelper.CheckEnvironment(); // 暂时注释掉，避免重复输出
            ...
            System.Diagnostics.Debug.WriteLine("✅ Win2DTextEditor 构造函数完成");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"❌ Win2DTextEditor 构造函数异常:");
            System.Diagnostics.Debug.WriteLine($"   类型: {ex.GetType().FullName}");
    ...
    private void OnThemeChanged(Windows.UI.Xaml.FrameworkElement sender, object args)
    {
        ApplyCurrentTheme();
    }
    ...
    private void Canvas_CreateResources(CanvasControl sender, Microsoft.Graphics.Canvas.UI.CanvasCreateResourcesEventArgs args)
    {
        try
        {
            System.Diagnostics.Debug.WriteLine("📦 Canvas_CreateResources 开始");
            ...
            System.Diagnostics.Debug.WriteLine("✅ Canvas_CreateResources 完成");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"❌ Canvas_CreateResources 异常: {ex.Message}");
            throw;
        }
    }
    ...
    private void Canvas_Draw(CanvasControl sender, CanvasDrawEventArgs args)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"❌ Canvas_Draw 异常: {ex.Message}");
            // 绘制错误时不抛出异常，避免崩溃
        }
    }
    ...
    private void Canvas_CharacterReceived(Windows.UI.Xaml.UIElement sender, CharacterReceivedRoutedEventArgs e)
    {
        // 处理字符输入
